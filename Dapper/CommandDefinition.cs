...
namespace Dapper
{
    ...
    public struct CommandDefinition
    {
        ...
        /// 
        /// </summary>
        /// <param name="totalRecord">影响记录数</param>
        internal void OnCompleted(int totalRecord=0)
        {
            (Parameters as SqlMapper.IParameterCallbacks)?.OnCompleted();
            SqlMapperTrace.ShellAfterCommandExecute(new TraceInfo()
            {
                Token = this.GetHashCode().ToString(),
                ExecuteTime = DateTime.Now,
                CommandText = CommandText,
                SqlParams=Parameters,
                IsStart=    SqlState.End,
                IsDbSucc=totalRecord>0
            }) ;
        }
        ...
        internal IDbCommand SetupCommand(IDbConnection cnn, Action<IDbCommand, object> paramReader)
        {
            ...
            SqlMapperTrace.ShellBeforeCommandExecute(new TraceInfo() { 
                Token=this.GetHashCode().ToString(),
                CommandText=CommandText,
                ExecuteTime=DateTime.Now,
                SqlParams=Parameters,
                IsStart=SqlState.Start
            });
