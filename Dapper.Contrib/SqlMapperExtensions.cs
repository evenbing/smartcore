    ...
    public static partial class SqlMapperExtensions
    {
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                string sqlColumn = GetSqlColumnList(type, connection);
                sql = $"select {sqlColumn} from {name} where {key.Name} = @id";
        ...
        public static IEnumerable<T> GetAll<T>(this IDbConnection connection, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(cacheType.TypeHandle, out string sql))
            {
                ...
                var name = GetTableName(type);
                string sqlColumn = GetSqlColumnList(type, connection);
                sql = $"select {sqlColumn} from {name}";
        ...
        /// <returns></returns>
        private static string GetSqlColumnList(Type type, IDbConnection connection)
        {
            ...
            return sbColumnList.ToString().TrimEnd(',');
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static Tuple<IEnumerable<T>, int> GetListByPage<T>(this IDbConnection connection, PageModel pageModel,IDbTransaction transaction = null, int? commandTimeout = null)
        { 
            ...
            var adapter = GetFormatter(connection);
            string sql = adapter.PackagePageSql(pageModel);
            int total = 0;
            using (var reader = connection.QueryMultiple(sql, pageModel.DynamicParam, transaction, commandTimeout))
            {
                total = reader.ReadFirst<int>(); 
                var result= reader.Read<T>();
                return new Tuple<IEnumerable<T>, int>(result, total);
            }
        }
        /// <summary>
...
public partial interface ISqlAdapter
{
    ...
    /// <param name="pageModel"></param>
    /// <returns></returns>
    string PackagePageSql(PageModel pageModel);
}
...
public partial class SqlServerAdapter : ISqlAdapter
{
    ...
    public  bool InsertByBulkCopy(SqlConnection connection, SqlTransaction transaction,DataTable dt)
    {
        if (dt.Rows.Count > 0)
        {
    ...
    /// <param name="pageModel"></param>
    /// <returns></returns>
    public string PackagePageSql(PageModel pageModel)
    {

        StringBuilder sb = new StringBuilder();
        if (!string.IsNullOrEmpty(pageModel.GroupBy))
        {
            sb.AppendFormat("SELECT COUNT(1) FROM (SELECT COUNT(1)  FROM {0} {1}  {2}) as acount;", pageModel.Tables, pageModel.ConditionWhere, pageModel.GroupBy);
        }
        else
        {
            sb.AppendFormat("SELECT COUNT(1)  FROM {0} {1};", pageModel.Tables, pageModel.ConditionWhere);
        }
        if (pageModel.PageIndex.Equals(1))
        {
            sb.AppendFormat("SELECT TOP  {0} {1}  FROM {2} {3} {4} {5} ;",
                pageModel.PageSize, pageModel.ReturnFields, pageModel.Tables, pageModel.ConditionWhere, pageModel.GroupBy, pageModel.OrderBy);
        }
        else
        {
            sb.AppendFormat("SELECT   {0}  FROM {1} {2}  {3}   offset {4} rows fetch next {5} rows only ;", pageModel.ReturnFields,
           pageModel.Tables, pageModel.ConditionWhere, pageModel.OrderBy
           , (pageModel.PageIndex - 1) * pageModel.PageSize, pageModel.PageSize);
        }
        return sb.ToString();
    }
}
...
public partial class MySqlAdapter : ISqlAdapter
{
    ...
    public string PackagePageSql(PageModel pageModel)
    {
        ...
        {
            sb.AppendFormat("SELECT   {0}  FROM {1} {2}  {3}   limit {4}  {5};", pageModel.ReturnFields,
           pageModel.Tables, pageModel.ConditionWhere, pageModel.OrderBy
           , (pageModel.PageIndex - 1) * pageModel.PageSize, pageModel.PageSize);
        }
        return sb.ToString();
